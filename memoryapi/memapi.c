...
char *print_errors(mem_errors_p errors)
{
    ...
    for (int i = 0; i < errors->num_errors; i++)
    {
        ...
        if (i != errors->num_errors - 1)
            sprintf_s(errors->error_str, 1024, "\r\n");
    }
...
LIB_EXPORT mem_mapview_p mem_create_view(mem_mapping_p map, int offset_hi, int offset_lo, size_t size)
{
    ...
    //     int infoSize = VirtualQuery(view->data, &info, sizeof(info));
...
LONGLONG  make_longlong(int hi, int lo) {
    LONGLONG  ret = hi;
    ret = (ret << sizeof(int));
    ret = ret | (LONGLONG)lo;
...
LIB_EXPORT bool mem_write_to_view(mem_mapview_p view, int offset_hi, int offset_lo, unsigned char *bytes, int len)
{
    __try
    {
        LONGLONG  offset = make_longlong(offset_hi, offset_lo);
        *(int*)((char*)view->data + offset) = len;
        int sizeOfLen = sizeof(int) / sizeof(char);
        for (int i = 0; i < len; i++)
        {
...
LIB_EXPORT char *mem_read_view(mem_mapview_p view, int offset_hi, int offset_lo, size_t *read)
{
    ...
    {
        LONGLONG  offset = make_longlong(offset_hi, offset_lo);
        int len = *(int*)((char*)view->data + offset);
        char *ret = (char *)malloc(sizeof(char) * len);
        int sizeOfLen = sizeof(int ) / sizeof(char);
        for (int i = 0; i < len; i++)
        {
            char byte = ((char *)(view->data))[i + offset + sizeOfLen];
            ret[i] = byte;
            *read += 1;
        }
        return ret;
